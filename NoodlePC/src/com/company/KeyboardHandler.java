...
class KeyboardHandler {
	...
	KeyboardHandler() {
		try {
			...
			System.out.println("Error creating Keyboard Robot");
		}
	}
	...
	void type(CharSequence characters) {
		...
		String stringChars = characters.toString().replace("KEYCODE", "VK");
		stringChars = stringChars.replace("RIGHT", "");
		stringChars = stringChars.replace("LEFT", "");
		stringChars = stringChars.replace("__", "_");
		stringChars = stringChars.charAt(stringChars.length() - 1) == '_' ? stringChars.substring(0, stringChars.length() - 1) : stringChars;
		stringChars = stringChars.charAt(0) == '_' ? stringChars.substring(1, stringChars.length()) : stringChars;
	...
	static void cleanup(){
		Robot cleanupRobo = null;
		
		try{
			cleanupRobo = new Robot();
